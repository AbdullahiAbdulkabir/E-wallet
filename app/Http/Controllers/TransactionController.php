...
class TransactionController extends Controller
{
    ...
    public function postwithdrawl(Request $request)
    {
        $client = auth()->guard('client')->user();
      
        
        if(Account::where('withdrawal', '<', Carbon::now())->where('client_id',$client->id)->count())
        {
            $client = auth()->guard('client')->user();
            $sender_account=Account::where('client_id',$client->id)->first();
            $balance=$sender_account->balance;
            ...
            if($remaining_balance > 0){
                ...
                $transaction->currency='usd';
                $transaction->description=$request->input('description');
                $transaction->datewithdrawal=$request->input('withdrawldate');
                // dd($transaction->datewithdrawal);
                $transaction->fee='0';
                $transaction->client_id=$id =auth()->guard('client')->user()->id;
                $transaction->sender=auth()->guard('client')->user()->names;
                $transaction->status='completed';
                ...
                \Session::put('success','Funds withdrawl successfully');
                return redirect()->route('home');
            }else
            {
                \Session::put('error','Insufficient funds');
        ...
        {
            \Session::put('error','It not yet withdrawal day');
